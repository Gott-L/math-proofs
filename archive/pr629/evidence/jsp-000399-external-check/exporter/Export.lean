  ...
  for (_, constInfo) in env.constants do
    if let .recInfo recVal := constInfo then
      for indName in recVal.all do
        -- Lean 4.19 NameMap is an RBMap, using find? and insert.
        let recNames := (recursorMap.find? indName).getD NameSet.empty
...
def removeMData (e : Expr) : M Expr := do
  ...
  | .letE n d v b _ =>
    ...
    -- Lean 4.19's updateLet! preserves the old flag and has no flag argument.
    -- Construct the expression explicitly to retain the upstream normalization.
    pure <| Expr.letE n (← removeMData d) (← removeMData v) (← removeMData b) false
  | .forallE _ d b _ =>
